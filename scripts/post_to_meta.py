...
def raw_github_url(filename):
    return f"{MEDIA_REPO_RAW}/main/week1/{filename}"


def post_to_instagram(caption, image_filename=None, video_filename=None, is_reel=True, thumb_offset_ms=None):
    ig_id = os.environ["META_IG_BUSINESS_ID"]
    token = os.environ["META_USER_ACCESS_TOKEN"]

    params = {"caption": caption, "access_token": token}
    if video_filename:
        params["video_url"] = raw_github_url(video_filename)
        ...
        params["image_url"] = raw_github_url(image_filename)
    else:
...
def post_to_facebook(message, video_filename=None, image_filename=None):
